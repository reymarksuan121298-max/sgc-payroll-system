...
export default function Sidebar({ isOpen, toggleSidebar }) {
  ...
  return (
    <>
      ...
      `}>
        <div className="p-6 pt-10 flex flex-col items-center lg:items-start">
          <img src="/logo.png" alt="SGC Logo" className="h-16 w-auto mb-2" />
        ...
        <nav className="flex-1 px-3 space-y-1 mt-4 overflow-y-auto">
          {menuItems.map((item) => (
            <div
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 ${location.pathname === item.path
...
function IOSDropdown({ label, icon, isOpen, onClick, children }) {
  return (
    <div className="space-y-1">
      <button
        onClick={onClick}
        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${isOpen
...
function SubItem({ label, active, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`px-3 py-2 text-[13px] cursor-pointer font-bold transition-all rounded-lg ${active
